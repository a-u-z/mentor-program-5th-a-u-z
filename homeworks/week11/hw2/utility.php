<?php
    ...
    function get_author_data($author) {
        global $connect;
        $sql = "select * from a_u_z_author where author = ?";
        $select_user = $connect -> prepare($sql);
        $select_user ->bind_param('s', $author);
        $middle_man = $select_user -> execute();
        $middle_man = $select_user ->get_result();
        $get_author_data_result = $middle_man->fetch_assoc();
        return $get_author_data_result; // 裡面有 id, author, password_hash
    }
