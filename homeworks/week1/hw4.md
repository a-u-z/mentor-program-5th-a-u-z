...
## 廣場，存檔的前哨站
這個城鎮被我們監控了，不過有些東西（檔案）流入（新增）流出（刪除），並不是重要的，所以我們就不用理它，如果是重要的事情我們就要加入暫存區（ staging area ），可以想像成是一個廣場，暫時放在那裡，等到很多的時候再一起搬到儲存庫中。

* 一個個加入到廣場吧
	* 詠唱 `git add 檔案名稱.副檔名` ，就可以把它加入廣場（暫存區），可以等到廣場有很多檔案後，再一次處理。
	* 除了放到廣場外，還有一個功能是可以追蹤它，加入過一次廣場後，之後可以不用放到廣場，可以直接放入儲存庫，可是如果連一次都沒有放到廣場的話，就一直不是追蹤中，就沒辦法一次放到儲存庫中。
* 查看我的廣場
...
* 	逃離視線
	*  詠唱 `git rm --cached 檔案名稱.副檔名`，可以從此不被追蹤


## 放入儲存庫，存檔
要把廣場的東西，放入儲存庫，才算是真正完成存檔，保存下來。

* 儲存庫之前
	* 可以先詠唱 `git diff` ，來看看做了哪些更動。
	*  詠唱 `git diff HEAD`，比較目前工作狀態與 HEAD（目前所在分支的最新 commit）之間的差異
* 放入儲存庫
	* 要詠唱`git commit -m "加入一些說明"`，這樣就可以把廣場（暫存區）的檔案放入儲存庫（存檔），加入一些說明是要對於這次存檔做一些說明，像是修了什麼 bug ，或是加了什麼更改，這很重要，因為這樣別人才會知道這次存檔，跟上次存檔，哪裡不同？
* 真的有放入儲存庫嗎？
	* 詠唱 `git log` ，就可以看到檔案放入儲存庫的編號，可以從這裡確認。
...
* 反悔
	* 詠唱`git reset HEAD^ --soft`，可以把 commit 倒退一步，但是檔案的內容不會回溯。
	*  詠唱`git reset HEAD^ --hard`，可以把 commit 倒退一步，然後檔案的內容也會回溯。

* 先放廣場很麻煩？
	* `git commit -am "加入一些說明"`，這樣就可以直接跳過放入廣場（暫存區）這個步驟，直接放入儲存庫（存檔）。但是要先被 add 過的才可以喔。
...
## 我曾經是你，但你未來是我（談分支）
可以創造這個城鎮的複製體，我們俗稱分支，然後去培養它，將來就可以用來取代原本的。

* 複製體（分支） 
	* 詠唱 `git branch 分支名稱` ，分支名稱打想要輸入的分支名稱，就可以建立一個城鎮的複製體，等等就可以開始實驗了。
* 幫現複製體（分支） 改名字
	* 詠唱 `git branch -m 新的名稱`，新的名稱輸入想要的名字，就可以幫現在這個空改名字。
* 老虎？老鼠？傻傻分不清楚
	*  詠唱 `git branch -v` ，可以看到有分岔出幾個分支
* 摧毀複製體（分支）
...
## 城鎮實驗室－－進階
上面都有做到的話，應該會是有 master ，跟複製體（分支），兩個分支，每一個分支裡面有會有不同的時間點的存檔。
...
* 時間跳躍
	...
	* 如果跳到過去，然後改了檔案也存檔了，這時候會產生一個新的過去？（這樣表達不知道好不好懂）就是屬於 detached HEAD ，但是這個過去並不屬於 master ，也不屬於複製體（分支），如果沒有設定名稱也沒有記住它的版本號的話，之後就找不到它，所以可以幫它設定成一個分支，給他一個名稱，這樣它就可以連結到分支樹裡面了。
...
* 能量吸收
	* 詠唱 `git merge 分支名稱` ，分支名稱要放想要吸進來的分支名稱，這樣就可以把它的一切吸進來了，然後 git 會自動幫我們 commit 一個新的版本。
...
*  將別人的複製到自己的電腦上
	*  詠唱`git clone 網址`，這只是你有一個一模一樣的東西，像是從網路上下載一份文件，你可以在自己的電腦裡面任意更改，不過並不會影響到「名人堂」上面那一份。
*  fork 叉子？
	*  可以 fork 別人的專案，做的事情是，把別人的專案用一個一模一樣的到自己的 repository ，然後再下載到自己的本機上面，當我們改了自己本機的專案，可以 push 到自己的遠端上面，然後可以發一個 pull request 給原本的作者，讓他看看新的改動他喜不喜歡，如果喜歡的話可以 merge 。簡而言之，可以當成如果想要跟原本的作者有互動，就用 fork ，如果不想，就用 clone
...
https://kaochenlong.com/2017/08/11/git-on-modern-web/

滿棒的流程圖：https://hackmd.io/@bngandan/BJIWpT168

https://www.toolsqa.com/git/difference-between-git-clone-and-git-fork/

本機、遠端流程圖：
https://hackmd.io/@U5Og9n6_Rs6IpjiKMQK61A/git
